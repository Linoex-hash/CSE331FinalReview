    ...
    \begin{frame}{Introduction}
        \begin{columns}
            \begin{column}{0.5\textwidth}
                ...
                \onslide<3->{See \hyperlink{def:greedystaysahead}
        ...
        \end{columns}
    \end{frame}
    \begin{frame}{Examples of Greedy Algorithms}
        \only<-2>{\href{https://en.wikipedia.org/wiki/Dijkstra\%27s\_algorithm}{Dijkstra's Algorithm}:
            \begin{itemize}
                \item<1-> Given a weighted graph with no negative weights, one can greedily find the shortest path in $O((m + n)\log n)$ time using a heap/priority queue. This is with $n$ vertices and $m$ edges.
                \item<2-> Useful for things like driving directions or fewest connecting flights.
            \end{itemize}
        }
        \bigskip

        \only<3->{\href{https://en.wikipedia.org/wiki/Prim\%27s\_algorithm}{Prim's Algorithm}:
            \begin{itemize}
                \item<3-> Given a weighted graph, one can generate a minimum spanning tree (a tree with the smallest/cheapest total edge cost) in $O(n^2)$ time with $n$ vertices. 
                Note that this runtime assumes an adjacency matrix representation for the graph.
                \item<4-> Refer to the wikipedia page for better complexities for graphs with different representations.
                \item<5-> Useful for things like network topologies for cheapest communication between two devices.
            \end{itemize}
        }
    \end{frame}
    \end{document}